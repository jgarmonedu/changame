<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Update').' '. __('Contribution') }}
        </h2>
    </x-slot>
    <form method="POST" action="/my/product/{{ $product->id }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="shadow border-b border-gray-200 sm:rounded-lg p-4 w-3/4 m-auto">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                @if ($product->images()->count()>0)
                    <div class="mt-2 lg:col-span-2 border rounded-md shadow-sm flex flex-row justify-center">
                        @foreach ($product->images()->get() as $image)
{{--                         <form method="POST" action="/my/product/image/{{ $image->id }}" enctype="multipart/form-data">--}}
{{--                             @csrf--}}
{{--                             @method('DELETE')--}}
{{--                             <div>--}}
{{--                                 <x-form.text-input id="name" type="hidden" name="image" :value="$image->id"/>--}}
{{--                                 <button type="submit" class="position-absolute bg-white p-2 m-3 rounded">--}}
{{--                                     <i class="fa fa-trash m-1" aria-hidden="true"></i>--}}
{{--                                 </button>--}}
                                 <img src="/storage/{{$image->thumbnail}}" class="h-48 w-full object-cover md:h-full md:w-48 p-2 gap-5 rounded-4">
{{--                             </div>--}}
{{--                         </form>--}}
                        @endforeach
                    </div>
